<?php
/**
 * TransferLocation Model
 * Master list of pickup/dropoff locations used by Transfer routes.
 */
class TransferLocation
{
    private $conn;
    private $table = 'transfer_locations';
    ...
    public function create($data)
    {
        $stmt = $this->conn->prepare(
            "INSERT INTO {$this->table} (name, is_active, sort_order)
    ...
    public function update($id, $data)
    {
        $stmt = $this->conn->prepare(
            "UPDATE {$this->table}
             SET name = :name, is_active = :is_active, sort_order = :sort_order
             WHERE id = :id"
        );
        $stmt->bindValue(':name', $data['name']);
        $stmt->bindValue(':is_active', isset($data['is_active']) ? (int)$data['is_active'] : 1, PDO::PARAM_INT);
